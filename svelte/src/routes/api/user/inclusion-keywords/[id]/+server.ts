// src/routes/api/user/inclusion-keywords/+server.ts
...
export async function GET({ locals }) {
  ...
  const keywords = await db
    .select()
    .from(inclusionKeywords)
    .where(inclusionKeywords.userId.eq(userId))
    .orderBy(inclusionKeywords.createdAt, 'desc');
