...
def get_jobs_from_db():
    """
    Get jobs from database with filters sent from frontend.
    """
    data = request.json or {}
    ...
    if keyword:
        ...
        query = query.filter(or_(
            Job.headline.ilike(pattern),
            Job.description.ilike(pattern),
            Job.employer_name.ilike(pattern)
        ))

    # Exclude filters
    exclude_keywords = [f.value.lower() for f in JobFilter.query.filter_by(filter_type="exclude").all()]
    for ex in exclude_keywords:
        pattern = f"%{ex}%"
        query = query.filter(~Job.headline.ilike(pattern))
